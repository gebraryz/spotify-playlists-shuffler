...
export const ShufflerUserPlaylistsItemShuffleButton: FC<{
  id: string;
  totalTracks: number;
  accessToken: string;
}> = ({ id, totalTracks, accessToken }) => {
  const fetchAllTracks = useCallback(async (playlistId: string) => {
  ...
  const [selectedShuffleOption, setSelectedShuffleOption] = useState<ShuffleOptions>();
  const [isShuffling, setIsShuffling] = useState(false);
  const { toast } = useToast();

  const shuffleOptions = useMemo(() => [
    { label: t('shuffle_options_date_playlist_asc'), value: ShuffleOptions.DatePlaylistAsc },
    { label: t('shuffle_options_date_playlist_desc'), value: ShuffleOptions.DatePlaylistDesc },
    { label: t('shuffle_options_length_asc'), value: ShuffleOptions.LengthAsc },
    { label: t('shuffle_options_length_desc'), value: ShuffleOptions.LengthDesc },
    { label: t('shuffle_options_popular'), value: ShuffleOptions.Popular },
    { label: t('shuffle_options_random'), value: ShuffleOptions.Random },
  ], [t]);

  const sortTracks = useCallback((tracks: SpotifyApi.PlaylistTrackObject[]) => {
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: async () => {
      ...
      const trackUpdates = sortedTracks.map((sortedTrack, i) => {
        if (!sortedTrack) {
          return null;
        }
        ...
        if (initialTrackIndex !== -1) {
          return request.put(`playlists/${id}/tracks`, {
            headers: { Authorization: `Bearer ${accessToken}` },
            body: JSON.stringify({
